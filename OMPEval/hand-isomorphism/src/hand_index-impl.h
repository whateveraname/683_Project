...
struct hand_indexer_s {
  ...
  uint_fast32_t (* configuration[MAX_ROUNDS])[_SUITS];
  uint_fast32_t (* configuration_to_suit_size[MAX_ROUNDS])[_SUITS];
  hand_index_t * configuration_to_offset[MAX_ROUNDS];
};

struct hand_indexer_state_s {
  uint_fast32_t suit_index[_SUITS];
  uint_fast32_t suit_multiplier[_SUITS];
  uint_fast32_t round, permutation_index, permutation_multiplier;
  uint32_t used_ranks[_SUITS];
};
