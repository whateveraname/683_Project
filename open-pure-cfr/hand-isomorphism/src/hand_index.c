...
static uint8_t nth_unset[1<<_RANKS][_RANKS];
static bool equal[1<<(_SUITS-1)][_SUITS];
static uint_fast32_t nCr_ranks[_RANKS+1][_RANKS+1], rank_set_to_index[1<<_RANKS], index_to_rank_set[_RANKS+1][1<<_RANKS], (*suit_permutations)[_SUITS];
static hand_index_t nCr_groups[MAX_GROUP_INDEX][_SUITS+1];
static void __attribute__((constructor)) hand_index_ctor() {
  for(uint_fast32_t i=0; i<1<<(_SUITS-1); ++i) {
    for(uint_fast32_t j=1; j<_SUITS; ++j) {
      equal[i][j] = i&1<<(j-1);
    }
  }

  for(uint_fast32_t i=0; i<1<<_RANKS; ++i) {
    for(uint_fast32_t j=0, set=~i&(1<<_RANKS)-1; j<_RANKS; ++j, set&=set-1) {
      nth_unset[i][j] = set?__builtin_ctz(set):0xff;
    }
  }

  nCr_ranks[0][0]     = 1;
  for(uint_fast32_t i=1; i<_RANKS+1; ++i) {
  ...
  for(uint_fast32_t i=1; i<MAX_GROUP_INDEX; ++i) {
    ...
    for(uint_fast32_t j=1; j<(i<(_SUITS+1)?i:(_SUITS+1)); ++j) {
  ...
  for(uint_fast32_t i=2; i<=_SUITS; ++i) {
    num_permutations *= i;
  }

  suit_permutations = calloc(num_permutations, _SUITS*sizeof(uint_fast32_t));
  
  for(uint_fast32_t i=0; i<num_permutations; ++i) {
    for(uint_fast32_t j=0, index=i, used=0; j<_SUITS; ++j) {
      uint_fast32_t suit = index%(_SUITS-j); index /= _SUITS-j;
...
void enumerate_configurations_r(uint_fast32_t rounds, const uint8_t cards_per_round[], 
  ...
  if (suit == _SUITS) {
    ...
    uint_fast32_t max = _RANKS-used[suit];
    if (remaining < max) {
      max = remaining;
    }
   
    uint_fast32_t previous = _RANKS+1;
...
void enumerate_configurations(uint_fast32_t rounds, const uint8_t cards_per_round[],
    void (*observe)(uint_fast32_t, uint_fast32_t[], void*), void * data) {
  uint_fast32_t used[_SUITS] = {0}, configuration[_SUITS] = {0};
  enumerate_configurations_r(rounds, cards_per_round, 0, cards_per_round[0], 0, (1<<_SUITS) - 2, used, configuration, observe, data);
}
...
void tabulate_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
  ...
  for(; id>0; --id) {
    for(uint_fast32_t i=0; i<_SUITS; ++i) {
...
out:;
  ...
  for(uint_fast32_t i=0; i<_SUITS;) {
    hand_index_t size = 1;
    for(uint_fast32_t j=0, remaining=_RANKS; j<=round; ++j) {
      uint_fast32_t ranks = configuration[i]>>ROUND_SHIFT*(indexer->rounds-j-1)&ROUND_MASK;
      size *= nCr_ranks[remaining][ranks];
      remaining -= ranks;
    }
    assert(size+_SUITS-1 < MAX_GROUP_INDEX);
    
    uint_fast32_t j=i+1; for(; j<_SUITS && configuration[j] == configuration[i]; ++j) {} 
...
void enumerate_permutations(uint_fast32_t rounds, const uint8_t cards_per_round[],
    void (*observe)(uint_fast32_t, uint_fast32_t[], void*), void * data) {
  uint_fast32_t used[_SUITS] = {0}, count[_SUITS] = {0};
...
void count_permutations(uint_fast32_t round, uint_fast32_t count[], void * data) {
  ...
  for(uint_fast32_t i=0; i<=round; ++i) {
    for(uint_fast32_t j=0, remaining=indexer->cards_per_round[i]; j<_SUITS-1; ++j) {
...
void tabulate_permutations(uint_fast32_t round, uint_fast32_t count[], void * data) {
  ...
  uint_fast32_t pi[_SUITS];
  for(uint_fast32_t i=0; i<_SUITS; ++i) {
    pi[i] = i;
  }

  for(uint_fast32_t i=1; i<_SUITS; ++i) {
...
bool hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[], hand_indexer_t * indexer) {
  ...
  for(uint_fast32_t i=0; i<rounds; ++i) {
    ...
    indexer->configuration[i]              = calloc(indexer->configurations[i], _SUITS*sizeof(uint_fast32_t));
    indexer->configuration_to_suit_size[i] = calloc(indexer->configurations[i], _SUITS*sizeof(uint_fast32_t));
...
hand_index_t hand_index_next_round(const hand_indexer_t * indexer, const uint8_t cards[], hand_indexer_state_t * state) {
  ...
  uint_fast32_t ranks[_SUITS] = {0}, shifted_ranks[_SUITS] = {0};
  for(uint_fast32_t i=0; i<indexer->cards_per_round[round]; ++i) {
    assert(cards[i] < _CARDS);                 /* valid card */
  ...
  for(uint_fast32_t i=0; i<_SUITS; ++i) {
    ...
    state->suit_multiplier[i] *= nCr_ranks[_RANKS-used_size][this_size];
    state->used_ranks[i]      |= ranks[i];
  }

  for(uint_fast32_t i=0, remaining=indexer->cards_per_round[round]; i<_SUITS-1; ++i) {
  ...
  hand_index_t suit_index[_SUITS], suit_multiplier[_SUITS];
  for(uint_fast32_t i=0; i<_SUITS; ++i) {
...
#define swap(u, v) \
  ...
  for(uint_fast32_t i=0; i<_SUITS;) {
    ...
    if (i+1 < _SUITS && equal[equal_index][i+1]) {
      if (i+2 < _SUITS && equal[equal_index][i+2]) {
        if (i+3 < _SUITS && equal[equal_index][i+3]) {
          /* four equal suits */
...
bool hand_unindex(const hand_indexer_t * indexer, uint_fast32_t round, hand_index_t index, uint8_t cards[]) {
  ...
  hand_index_t suit_index[_SUITS];
  for(uint_fast32_t i=0; i<_SUITS;) {
    uint_fast32_t j=i+1; for(; j<_SUITS && indexer->configuration[round][configuration_idx][j] == indexer->configuration[round][configuration_idx][i]; ++j) {}
  ...
  for(uint_fast32_t i=0; i<_SUITS; ++i) {
    ...
    for(uint_fast32_t j=0; j<indexer->rounds; ++j) {
      ...
      uint_fast32_t round_size     = nCr_ranks[_RANKS-m][n]; m += n;
